...
- **`state_key`** — the atom key under which the plugin's state lives in the agent struct.
- **`actions`** — lists the action modules the plugin provides.
...
- `{:ok, :continue}` — pass through to normal routing
- `{:ok, {:continue, signal}}` — rewrite the signal before routing
- `{:ok, {:override, action}}` — bypass the router entirely
- `{:error, reason}` — abort signal processing
