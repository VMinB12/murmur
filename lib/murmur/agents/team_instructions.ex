defmodule Murmur.Agents.TeamInstructions do
  @moduledoc """
  ...
  within a Murmur workspace: the fire-and-forget `tell` pattern, team roster,
  and expected collaboration etiquette.

  Called by `MessageInjector` before every LLM turn so the roster is always
  up-to-date (agents can join or leave mid-conversation).
  """
  ...
  def build(workspace_id, my_display_name) do
    ...
    You are "#{my_display_name}", one of several AI agents collaborating in a Murmur workspace.
