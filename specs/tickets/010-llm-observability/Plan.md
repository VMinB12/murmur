...
2. **Child span instrumentation**: use the merged `jido_ai` request, LLM, and tool lifecycle telemetry to attach LLM and tool spans to the active turn trace with Murmur-specific metadata for agent identity, workspace or team correlation, session grouping, interaction grouping, and cross-agent causation.
3. **Payload capture and message rendering**: combine Jido.AI lifecycle telemetry, ReqLLM raw payload telemetry, and runtime `ai.*` signals so LLM child spans carry full input conversations and structured assistant outputs.
...
The latest `jido_ai` telemetry work changes the implementation path:

- ReAct now emits request, LLM, and tool lifecycle telemetry through canonical `[:jido, :ai, ...]` namespaces.
- ReAct also emits `ai.tool.started` signals in addition to the existing `ai.llm.delta`, `ai.llm.response`, `ai.tool.result`, and `ai.usage` signals.
- Murmur can now create child spans from telemetry start and terminal events without hot-fixing `jido_ai`.
...
- **Use upstream telemetry as the supported hook**: Murmur should compose with the new Jido.AI telemetry surface rather than patching dependency code or recreating a second runtime lifecycle.
...
| ReqLLM payloads are still not explicitly correlated to Jido LLM call ids | Medium | Medium | Correlate ReqLLM request events to the active Jido LLM lifecycle by request id and turn order, and cover multi-iteration ReAct flows with integration tests. |
