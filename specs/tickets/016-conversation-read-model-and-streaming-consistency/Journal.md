...
- Tightened the ticket and ADR after review to favor the stronger design: `jido_murmur` should own the conversation projector, connected UIs should consume one Murmur-owned conversation update contract, and all facts entering the projector should carry stable turn identity instead of leaving reduction semantics inside `WorkspaceLive`.
- Completed the implementation: projector-backed snapshots and canonical incremental updates now drive chat rendering, late update ordering is covered by regression tests, and the redundant `ChatStream` component plus generator surface were removed during cleanup.